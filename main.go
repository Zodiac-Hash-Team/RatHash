...
func main() {
	...
	if pflag.NArg() == 0 || *pHelp {
		fmt.Println(yell + "The currently non-cryptographic hashing algorithm." + zero + "\n\n" +
			"Usage:\n" +
	...
	case *pLength >= 256 && *pLength%64 == 0:
		break
	default:
		fmt.Println(purp + "Digest length in bits must be at least 256 and evenly divisible by 64." + zero)
		os.Exit(22)
	}
	for i := range pflag.Args() {
		...
		str := fmt.Sprintf("%0x", digest)

		if *pBase64 {
			str = base64.StdEncoding.EncodeToString(digest)
		}
		if *pString {
			path = fmt.Sprint(zero + "\"" + path + "\"")
		}
		switch {
		case quiet:
			fmt.Println(str)
		case *pTime:
			fmt.Println(yell + str + zero + " : " + und + path + zero + ", (" + delta + ")")
		default:
			fmt.Println(yell + str + zero + " : " + und + path + zero)
		}
	}
	...
	if quiet != true {
		switch {
		case readErrs == 1:
			fmt.Println("1 " + purp + "target is inaccessible, does it exist?" + zero)
		case readErrs > 1:
			fmt.Println(readErrs, purp+"targets are inaccessible, do they exist?"+zero)
		}
	}
	os.Exit(exitCode)
}
